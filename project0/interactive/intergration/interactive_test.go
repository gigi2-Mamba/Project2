...
import (
	...
	startup "project0/interactive/intergration/startup"
...
var (
	err0  error=errors.New("Error 1062 (23000): Duplicate entry '2-test' for key 'interactives.biz_idx'")
)
...
func (i *InteractiveTestSuite)TearDownTest()  {
	...
	assert.NoError(i.T(), err)
	err = i.db.Exec("truncate TABLE `user_read_bizs`").Error
	assert.NoError(i.T(), err)
	err = i.db.Exec("truncate TABLE `user_collect_bizs`").Error
	assert.NoError(i.T(), err)
...
func (s *InteractiveTestSuite) TestDislike() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			resp,err := svc.CancelLike(context.Background(),
...
func (s *InteractiveTestSuite) TestCollect() {
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			resp,err := svc.Collect(context.Background(),
